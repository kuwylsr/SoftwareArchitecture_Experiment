...
public class GraphVisualizationHelper {
	...
	public static void visualize(Graph<Vertex, Edge> g,edu.uci.ics.jung.graph.Graph<Vertex,Edge> G) {
		//初始化 GUI图结构
		...
		for(Edge e: g.edges()) { 		
			...
			if(g instanceof MovieGraph || g instanceof NetworkTopology) {			 	
				Set<Vertex> s= e.targetVertices();
				Iterator<Vertex> it = s.iterator();
				while(it.hasNext()) {
					vertices.add(it.next());
				}
			}else if(g instanceof  GraphPoet || g instanceof SocialNetwork) {
			...
			if(vertices.size()==2) {
				if(g instanceof MovieGraph || g instanceof NetworkTopology)
					G.addEdge(e, vertices.get(0),vertices.get(1));
				else if(g instanceof  GraphPoet || g instanceof SocialNetwork)	
					G.addEdge(e, vertices.get(0),vertices.get(1),EdgeType.DIRECTED);
			}else if(vertices.size()==1) {
				G.addEdge(e, vertices.get(0),vertices.get(0));
			}else {
				for(int i=0;i<vertices.size()-1;i++) {
					...
					G.addEdge(e1, list.get(0),list.get(1));
				}
			}
	    ...
	    Transformer<Vertex, Paint> vertexPaint = new Transformer<Vertex, Paint>(){
			...
			public Paint transform(Vertex v) {
				if(v instanceof Word || v instanceof Person) return Color.RED;           
			     ...
			     else if(v instanceof WirelessRouter) return Color.cyan;
