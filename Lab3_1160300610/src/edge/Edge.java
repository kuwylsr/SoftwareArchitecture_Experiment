...
public abstract class Edge {
   ...
   //总之不存在表示暴露。
	...
	/*
	 *如果是超边，vertices.size()>=2，该函数添加 vertices 中的所有节点到该超边；
	 *如果是有向边，vertices.size()=2，该操作将vertices 中的第一个元素作为 source，将第二个元素作为 target；
	 *如果是无向边，vertices.size()=2，无需考虑次序；
	 *如果是 loop，vertices.size()=1
	 * 
	 * @param 顶点集合
	 ...
	 * 
	 * @param 顶点
	 * @return 返回是否包含该顶点，是为true，否为false
	 */
	 ...
	 * 判断两个List是否一致
	 * 
	 * @return 两个List是否元素相同，及顺序一致
	 */
	public boolean list_equals(Collection<Vertex> vertices1,Collection<Vertex> vertices2) {
		if(vertices1.containsAll(vertices2)&&vertices2.containsAll(vertices1)) {
			return true;
		}else {
			return false;
		}
	...
	public boolean equals(Object b) {
		Edge other = (Edge)b;
		if(this.label.equals(other.label)&&this.weight==other.weight&&list_equals(this.vertices, other.vertices)) {
			return true;
		}else {
			return false;
		}
