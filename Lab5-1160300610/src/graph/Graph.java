...
public interface Graph<L, E> {
  ...
  /*
   * 从图中删除一个节点v。如果v是某条边的两端之一，则该边被删除； 如果某节点属于某条超边，若该节点删除后该条超边仍可合法存在，则该超边保留，否则就删除之
   * 
   * @param 所要删除的顶点
   * 
   ...
   * 如果与target相连的边包括无向边，则无向边的另一端节点也需包含在返回值Map中； 不需考虑超边。
   * 
   * @param 边的终点
   * 
  ...
  /*
   * 如果与source相连的边包括无向边，则无向边的另一端节点也需包含在返回值Map中； 不需考虑超边。
   * 
   * @param 边的起点
   * 
   * @return 返回一个Map，满足如果与source相连的边包括无向边，则无向边的另一端节点也需包含在返回值Map中； 不需考虑超边。
   */
