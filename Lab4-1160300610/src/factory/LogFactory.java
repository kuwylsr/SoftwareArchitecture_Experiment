...
public class LogFactory {
     ...
     * @return
     */
	public static Logger initLog(int type1_w,int type2_f,String name) {
...
//		addConsoleHandler(log, Level.ALL);  //不注释这句，还是在控制台输出两次。。。
		if(type1_w==1) {
			addFileHandle(type2_f, log, Level.ALL, "src/graph/"+name+"_e.txt");
		}else if(type1_w == 2) {
			addFileHandle(type2_f, log, Level.ALL, "src/graph/"+name+"_o.txt");
		}
		
		//g.setUseParentHandlers(false); //设置false，防止控制台输出两次信息
     ...
     * @return
     */
	public static Logger Get_Logger(int type1_w,int type2_f,String name) {
		return initLog(type1_w,type2_f,name);
	}
	/**
     * 为log设置等级
     * 
    		...
    		if(type == 1) {  //图的文件异常日志格式
    			fileHandler.setFormatter(new Formatter() {				 
    				...
    				public String format(LogRecord record) {
    					...
    					+"发生错误的文件名： "+information[0]+"\r\n"
    					+"发生错误的文件内容： "+information[1]+"\r\n"					    					
    					+"发生错误的类名： "+record.getSourceClassName()+"\r\n"
